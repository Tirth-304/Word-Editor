...
const TextForm = (props) => {
    ...
    return (
        ...
        <div className="container my-3" style={{color: props.mode==='dark'?'white':'#042743'}}>
            ...
            <p> Number of Words is - {text.split(/\s+/).filter((element)=>{return element.length!==0}).length} & Number of characters is -  {text.length}</p>
            <p>{0.008 *  text.split(/\s+/).filter((element)=>{return element.length!==0}).length} Minutes read</p>
            <h2>Preview</h2>
            <p>{text.length>0?text:"Nothing to preview!"}</p>
        </div>
        <div className= {`form-check form-switch container my-3 text-${props.mode==='light' ? 'dark' : 'light'}`}>
            <input className="form-check-input" onClick={props.toggleMode} type="checkbox" role="switch" id="flexSwitchCheckDefault"/>
            <label className="form-check-label" htmlFor="flexSwitchCheckDefault"> Use Dark Mode </label>
        </div>
        </>
    )
}
